...
def test_class_probabilities(whichClass):
    ...
    with torch.no_grad():
        for data in testloader:
            ...
            actuals.extend( (labels.view_as(predicted) == whichClass).cpu().numpy() )
...
probabilities_list = []
with torch.no_grad():
    test_preds = get_all_preds(model, testloader).to(device)

    stacked = torch.stack(
        (
            torch.as_tensor(testset.targets).to(device)
